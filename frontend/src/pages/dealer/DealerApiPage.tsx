...
function EndpointDoc({
  ...
  const base = getDealerApiBaseUrl();
  return (
...
export default function DealerApiPage() {
  ...
  const apiBase = getDealerApiBaseUrl();
  ...
  const loadCreds = useCallback(() => {
    api.get('/dealer/api/credentials').then((res) => {
  ...
  useEffect(() => {
    if (user?.role === 'dealer') {
      loadCreds();
      api.get('/dealer/api/logs').then((res) => setLogs(res.data.data)).catch(console.error);
      api.get('/dealer/api/stats').then((res) => setStats(res.data.data)).catch(console.error);
    }
  }, [user, loadCreds]);

  const regenerate = async () => {
    if (!confirm('Regenerating keys will invalidate your current API credentials. Continue?')) return;
    const res = await api.post('/dealer/api/regenerate');
  ...
  const saveSettings = async () => {
    ...
    try {
      const ips = ipWhitelist
        ...
        .map((s) => s.trim())
        .filter(Boolean);
      await api.put('/dealer/api/settings', { ipWhitelist: ips, webhookUrl: webhookUrl || '' });
    ...
    <DashboardLayout role="dealer">
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-400">
          Integrate data purchases into your app using your dealer wallet balance.
        </p>
      </div>
...
Headers:
  ...
  x-secret-key: ${creds.secretKey || 'YOUR_SECRET_KEY'}`} />
          ...
          <div className="space-y-4">
            ...
            <EndpointDoc
              method="GET"
              path="/wallet"
              description="Returns your current dealer wallet balance in GHS. Purchases are debited from this balance."
              response={`{
  "success": true,
  "data": { "balance": 150.50 }
}`}
  ...
  "data": ["MTN", "Telecel", "AirtelTigo"]
            ...
            <EndpointDoc
              ...
              description="Lists enabled data bundles. Optional query parameter network filters by MTN, Telecel, or AirtelTigo. Prices shown are dealer prices (debited from your wallet)."
              response={`{
  "success": true,
  "data": [
    {
      ...
      "dealerPrice": 4.73
            ...
            <EndpointDoc
              method="POST"
              path="/purchase"
              description="Buy a single data bundle. The packageId comes from GET /packages. recipientPhone must be a valid Ghana number (10 digits, starts with 0). Your wallet is debited the dealerPrice immediately."
              request={`{
  ...
  "data": [
    ...
    { "orderId": "ORD-...", "recipientPhone": "0559876543", "status": "pending", ... }
            ...
            <EndpointDoc
              method="GET"
              path="/orders/:orderId"
              description="Check the status of an order you created. Only returns orders belonging to your dealer account."
              response={`{
  "success": true,
  "data": {
    "orderId": "ORD-M1ABC2-XY9Z",
