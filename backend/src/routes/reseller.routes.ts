...
router.get('/dashboard', asyncHandler(async (req: AuthRequest, res) => {
  ...
  const [
    ...
    Order.countDocuments({ resellerId, status: { $in: ['pending', 'processing'] } }),
...
const getCustomPrice = (user: AuthRequest['user'], packageId: string): number | undefined => {
...
router.get('/store', asyncHandler(async (req: AuthRequest, res) => {
  ...
  res.json({
    ...
    data: {
      ...
      storeUrl: `${env.frontendUrl}/store/${user.resellerStore.slug}`,
    },
  });
}));
...
const RESELLER_NETWORKS = ['MTN', 'Telecel', 'AirtelTigo'] as const;

// Price management
router.get('/prices', asyncHandler(async (req: AuthRequest, res) => {
  const packages = await Package.find({
    isEnabled: true,
    network: { $in: RESELLER_NETWORKS },
  }).sort({ network: 1, sortOrder: 1 });

  const data = packages.map((pkg) => {
    const custom = getCustomPrice(req.user!, pkg._id.toString());
    return {
      _id: pkg._id,
      network: pkg.network,
      ...
      myPrice: custom ?? pkg.resellerBasePrice,
    };
  });
...
router.put('/prices/:packageId', asyncHandler(async (req: AuthRequest, res) => {
  ...
  if (!RESELLER_NETWORKS.includes(pkg.network as (typeof RESELLER_NETWORKS)[number])) {
  ...
  res.json({ success: true, message: 'Price updated', data: { packageId: pkg._id, price } });
}));
...
router.get('/orders', asyncHandler(async (req: AuthRequest, res) => {
  const orders = await Order.find({
    ...
    .limit(200)
    .select(
      'orderId customerEmail recipientPhone network bundleSize status totalAmount createdAt updatedAt'
    );
...
router.post('/withdrawals', asyncHandler(async (req: AuthRequest, res) => {
